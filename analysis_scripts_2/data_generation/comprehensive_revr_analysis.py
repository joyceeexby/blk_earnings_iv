...
class ComprehensiveREVRAnalysis:
    ...
    Uses ST/MT methodology (same as original system) for consistent REVR calculation.
    """
    ...
    def analyze_single_revr_event(self, ticker, earnings_date, analysis_days_before=30):
        """
        Analyze a single REVR event using ticker and ST/MT methodology.
        ...
        analysis_days_before : int
            Days before earnings to analyze (default 30)
        """
        try:
            print(f"🔍 Analyzing REVR (ST/MT) for {ticker} on {earnings_date}")
            ...
            # Calculate REVR using ST/MT methodology (same as original system)
            print(f"🔍 Calculating REVR using ST/MT methodology for ticker: {ticker}")
            revr_result = self.revr_analyzer.calculate_revr_st_mt_ratio(
                ticker=ticker,
                earnings_date=earnings_date,
                days_before=120,  # Extended for better MT calculation (same as original)
                days_after=60      # Extended for better MT calculation (same as original)
            )
            print(f"🔍 REVR ST/MT calculation result: {revr_result}")
            ...
            if revr_result is not None and isinstance(revr_result, dict):
                ...
                if revr_value is not None and not pd.isna(revr_value):
                    result = {
                        ...
                        'revr': revr_value,
                        'vol_st': revr_result.get('vol_st'),           # Short-term volatility
                        'vol_mt': revr_result.get('vol_mt'),           # Medium-term volatility
                        'normative_realized_vol': revr_result.get('normative_realized_vol'),  # Normative vol
                        'methodology': 'ST/MT Ratio (Expanding EWM)'   # Methodology identifier
                    }
                    print(f"✅ Successfully created REVR ST/MT result: {result}")
                    return result
                else:
    ...
    def save_final_results(self, results, prefix=""):
        ...
        # Reorder columns for better readability, including new ST/MT fields
        base_columns = ['ticker', 'season', 'year', 'quarter', 'earnings_date', 'analysis_date', 'revr']
        additional_columns = ['vol_st', 'vol_mt', 'normative_realized_vol', 'methodology']
        
        # Only include columns that exist in the data
        available_columns = [col for col in base_columns + additional_columns if col in df.columns]
        df = df[available_columns]
        
        # Save to CSV with updated prefix to reflect ST/MT methodology
        filename = f'data_files/bulk_revr_comprehensive_st_mt_{prefix}_comparison.csv'
        df.to_csv(filename, index=False)
        ...
        # ST/MT volatility statistics if available
        if 'vol_st' in df.columns and 'vol_mt' in df.columns:
            print(f"\n📊 ST/MT Volatility Statistics:")
            print(f"  Short-term Vol (ST) - Mean: {df['vol_st'].mean():.3f}, Std: {df['vol_st'].std():.3f}")
            print(f"  Medium-term Vol (MT) - Mean: {df['vol_mt'].mean():.3f}, Std: {df['vol_mt'].std():.3f}")
            print(f"  ST/MT Ratio - Mean: {df['revr'].mean():.3f}")
...
def main():
    ...
    try:
        ...
        # Run comprehensive analysis with static stock list using ST/MT methodology
        ...
        if results is not None:
            ...
            print(f"Results saved to: bulk_revr_comprehensive_st_mt_static_cusip_comparison.csv")
