...
class FixedRegressionAnalysis:
    ...
    def create_variables(self):
        ...
        self.create_enhanced_categorical_features()
        
        # Create factor-based features from existing data
        self.create_factor_features_from_existing()
    
    def create_enhanced_categorical_features(self):
        """
        Create enhanced categorical features using existing data only.
        """
        print("Creating enhanced categorical features...")
        
        # Earnings timing features
        self.data['earnings_day_of_week'] = self.data['earnings_date'].dt.dayofweek
        
        # Market regime indicators
        self.data['high_vol_regime'] = (self.data['revr'] > 
                                       self.data['revr'].quantile(0.75)).astype(int)
        self.data['high_ievr_regime'] = (self.data['ievr'] > 
                                        self.data['ievr'].quantile(0.75)).astype(int)
        
        # Price-based features
        self.data['price_level'] = pd.qcut(self.data['underlying_price'], 
                                          q=5, labels=['Very_Low', 'Low', 'Medium', 'High', 'Very_High'])
        
        # Volatility regime
        if 'vol_st' in self.data.columns and not self.data['vol_st'].isna().all():
            self.data['volatility_regime'] = pd.qcut(self.data['vol_st'], 
                                                    q=3, labels=['Low_Vol', 'Medium_Vol', 'High_Vol'])
        else:
            # Create a placeholder volatility regime if vol_st is not available
            self.data['volatility_regime'] = 'Medium_Vol'
        
        # Earnings season indicators
        self.data['earnings_season'] = self.data['quarter'].map({
            1: 'Q1_earnings', 2: 'Q2_earnings', 
            3: 'Q3_earnings', 4: 'Q4_earnings'
        })
        
        # Market conditions
        if 'spx_ievr' in self.data.columns and not self.data['spx_ievr'].isna().all():
            self.data['market_regime'] = (self.data['spx_ievr'] > 
                                         self.data['spx_ievr'].quantile(0.75)).astype(int)
        else:
            # Create a placeholder market regime if spx_ievr is not available
            self.data['market_regime'] = 0
        
        print(f"  Created: earnings_day_of_week, high_vol_regime, high_ievr_regime")
        print(f"  Created: price_level, volatility_regime, earnings_season, market_regime")
    
    def create_factor_features_from_existing(self):
        """
        Create factor-based features using existing data only.
        """
        print("Creating factor-based features from existing data...")
        
        # Size factor (price-based proxy)
        self.data['size_factor'] = np.log(self.data['underlying_price'])
        
        # Momentum factor (using existing price data)
        momentum_factor = self.data.groupby('ticker')['underlying_price'].pct_change(20)
        self.data['momentum_factor'] = momentum_factor.reset_index(level=0, drop=True)
        
        # Volatility factor (using existing volatility data)
        if 'vol_st' in self.data.columns and 'vol_mt' in self.data.columns:
            self.data['volatility_factor'] = self.data['vol_st'] / self.data['vol_mt']
        else:
            # Create a placeholder volatility factor if vol_st/vol_mt are not available
            self.data['volatility_factor'] = 1.0
        
        # Quality factor (earnings stability)
        quality_factor = self.data.groupby('ticker')['revr'].rolling(4).std()
        self.data['quality_factor'] = (1 / quality_factor).reset_index(level=0, drop=True)
        
        # Market beta factor (correlation with market)
        # Note: This requires spx_ievr to be available, so we'll create a simpler version
        if 'spx_ievr' in self.data.columns and not self.data['spx_ievr'].isna().all():
            market_beta = self.data.groupby('ticker')['revr'].rolling(20).corr(self.data['spx_ievr'])
            self.data['market_beta'] = market_beta.reset_index(level=0, drop=True)
        else:
            # Create a placeholder market beta if spx_ievr is not available
            self.data['market_beta'] = 0.0
        
        # Fill NaN values with median
        factor_columns = ['size_factor', 'momentum_factor', 'volatility_factor', 
                         'quality_factor', 'market_beta']
        for col in factor_columns:
            if col in self.data.columns:
                self.data[col] = self.data[col].fillna(self.data[col].median())
        
        print(f"  Created: size_factor, momentum_factor, volatility_factor, quality_factor, market_beta")
