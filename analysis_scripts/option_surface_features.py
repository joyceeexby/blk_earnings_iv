...
def extract_skew_feature(secid, earnings_date, db, n_lag=30):
...
def get_sp500_constituents(as_of_date, db_connection):
    """
    Fetches the S&P 500 constituents for a given date.

    Parameters:
    - as_of_date (str): The date for which to fetch the constituents (format: 'YYYY-MM-DD').
    - db_connection: A WRDS database connection object with a `.raw_sql()` method.

    Returns:
    - pandas.DataFrame: DataFrame containing permno, ticker, and comnam of S&P 500 constituents.
    """
    query = f"""
        ...
        LEFT JOIN crsp.stocknames b
         ...
         AND b.nameenddt =  '{as_of_date}'
        WHERE b.ticker IS NOT NULL
          AND a.ending = '{as_of_date}'
        ORDER BY b.ticker
    """
    return db_connection.raw_sql(query)

def extract_kurtosis_feature(secid, earnings_date, db, n_lag=30):
    query_date = get_relative_surface_date(secid, earnings_date, n_lag, db)
    if query_date is None:
        return None, None
...
def compute_option_surface_features(ticker_list, earnings_year, earnings_quarter, db, n_lag=30):
    """
    ...
    # Filter earnings data by year and quarter
    earnings_df = earnings_df[
        (earnings_df['fyearq'] == earnings_year) &
        (earnings_df['fqtr'] == earnings_quarter)
    ].copy()
    print(f"Filtered down to {len(earnings_df)} earnings records for Q{earnings_quarter} {earnings_year}.")
...
def monthly_iv_change_ratio_feature(secid, earnings_date, db, n_lag=30, monthly_lag=21):
    """
    ...
    - db: WRDS database connection.
    - n_lag (int): Days before earnings_date to compute latest IV (default=15).
...
def extract_term_diff_feature(secid, earnings_date, db, n_lag=30):
    """
...
def extract_smirk_feature(secid, earnings_date, db, n_lag=30):
    query_date = get_relative_surface_date(secid, earnings_date, n_lag, db)
    if query_date is None:
        return None, None
...
def compute_option_surface_features_no_quarter_filter(ticker_list, earnings_date, db, n_lag=30):
    """
    Compute option surface features for a list of tickers for a specific earnings date.
    This version does NOT filter by quarter - it uses the actual earnings date.

    Parameters:
    - ticker_list (list[str]): List of stock tickers to include
    - earnings_date (str): Specific earnings date to analyze
    - db: WRDS database connection
    - n_lag (int): Trading days before earnings to measure features
    ...
    - pandas.DataFrame: DataFrame with calculated features
    """
    print("Starting compute_option_surface_features_no_quarter_filter...")
    
    # Get earnings data for a wide range around the target date
    earnings_dt = pd.to_datetime(earnings_date)
    start_earnings_date = (earnings_dt - pd.Timedelta(days=30)).strftime('%Y-%m-%d')
    end_earnings_date = (earnings_dt + pd.Timedelta(days=30)).strftime('%Y-%m-%d')
    ...
    # Filter to the specific earnings date (exact match)
    earnings_df = earnings_df[
        pd.to_datetime(earnings_df['earnings_date']).dt.date == earnings_dt.date()
    ].copy()
    print(f"Filtered to {len(earnings_df)} earnings records for {earnings_date}.")
    
    if earnings_df.empty:
        print(f"No earnings data found for {earnings_date}")
        return None
