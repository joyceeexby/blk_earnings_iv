...
class AutomatedEarningsAnalysis:
    ...
    def analyze_single_event(self, ticker, earnings_date, analysis_days_before=30):
        ...
        # Calculate REVR
        print(f"\n1. Calculating REVR...")
        revr_results = self.revr_analyzer.calculate_revr(
            ticker=ticker,
        ...
        event_results = {
            ...
            'vol_t_minus_3': revr_results['vol_t_minus_3'],
            'vol_t_plus_4': revr_results['vol_t_plus_4'],
            'kink_tte': ievr_results['kink_info']['tte'],
            'kink_moneyness': ievr_results['kink_info']['moneyness'],
            'underlying_price': underlying_price,
            'revr_results': revr_results,
            'ievr_results': ievr_results
        }
    ...
    def analyze_multiple_events(self, ticker, start_date, end_date, analysis_days_before=30):
        ...
        if successful_events:
            # Create Events × 2 matrix
            results_df = pd.DataFrame([
                {
                    'earnings_date': event['earnings_date'],
                    'revr': event['revr'],
                    'ievr': event['ievr'],
                    'ratio': event['ievr'] / event['revr'],
                    'vol_t_minus_3': event['vol_t_minus_3'],
                    'vol_t_plus_4': event['vol_t_plus_4'],
                    'kink_tte': event['kink_tte']
                }
